...
class BlockchainController {
    ...
    // Endpoint that allows to Submit a Star, yu need first to `requestOwnership` to have the message (POST endpoint)
    submitStar() {
    ...
    validateChain(){
        this.app.post("/validateChain", async (req, res) => {
            try {
                ...
                if(errors.length)
                    return res.status(200).json({message: "Chain Invalid.", errors});
                 else
                     return res.status(404).send({message: "Chain Valid"});
            } catch (error) {
                return res.status(500).send("An error happened!");
            }
        })
